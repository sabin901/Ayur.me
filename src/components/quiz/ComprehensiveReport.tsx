...
import { Flower2 as Lotus, User, Sun, BookOpen, Download, RotateCcw, Brain, Activity, Utensils, Clock, TreePine, Target, Award, Shield, Save } from 'lucide-react';
import { Badge } from "@/components/ui/badge";
import { useAuth } from '@/lib/auth-context';
import { ApiError, assessments } from '@/lib/api';
import type { DoshaResult } from './DoshaQuiz';
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
            ...
            {!authLoading && user && (
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:opacity-60 transition-colors"
              >
                <Save className="w-4 h-4" />
                {saveStatus === 'saved' ? 'Saved' : saveStatus === 'saving' ? 'Saving…' : 'Save to my account'}
              </button>
            )}
            {!authLoading && !user && (
              <Link
          ...
          {saveMessage && (
            <p
              role="status"
              className={`mt-3 text-sm ${saveStatus === 'error' ? 'text-red-600' : 'text-emerald-700'}`}
            >
              {saveMessage}
            </p>
          )}
