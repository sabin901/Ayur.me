...
function getDefaultAchievements(): Achievement[] {
  return [
    ...
    },
    {
      id: 'dosha_balancer',
      name: 'Dosha Balancer',
      description: 'Log activities tailored to your primary Dosha 15 times',
      icon: '⚖️',
      unlocked: false,
      progress: 0,
      target: 15
    },
    {
      id: 'ojas_builder',
      name: 'Ojas Builder',
      description: 'Consistently log 2+ activities daily for a week to build immunity',
      icon: '🛡️',
      unlocked: false,
      progress: 0,
      target: 7
...
function calculateCurrentStreak(activities: WellnessActivity[]): number {
...
function updateAchievements(achievements: Achievement[], activities: WellnessActivity[]): Achievement[] {
  return achievements.map(achievement => {
    ...
    switch (achievement.id) {
      ...
      case 'lifestyle_change':
        ...
        break;
      case 'dosha_balancer':
        progress = activities.filter(a => a.dosha && a.completed).length;
        break;
      case 'ojas_builder':
        progress = Math.min(calculateCurrentStreak(activities), 7);
        break;
...
function getMockActivities(): WellnessActivity[] {
  const mock: WellnessActivity[] = [];
  const now = new Date();
  
  const activityTypes: ('yoga' | 'meditation' | 'diet' | 'herbs' | 'lifestyle' | 'mental_health')[] = 
    ['yoga', 'meditation', 'diet', 'herbs', 'lifestyle', 'mental_health'];
  
  const moods: ('great' | 'good' | 'neutral' | 'poor')[] = ['great', 'good', 'good', 'neutral'];
  ...
  for (let i = 30; i >= 0; i--) {
    ...
    for (let j = 0; j < numActivities; j++) {
      ...
      mock.push({
        ...
        dosha: doshas[Math.floor(Math.random() * doshas.length)]
      });
    }
  }
  return mock;
...
export function useProgress() {
  const [progress, setProgress] = useState<ProgressData>(() => {
    ...
    // Force inject mock data if we have no activities or if the old 14-day mock data is present
    if (!parsed || !parsed.activities || parsed.activities.length < 40) {
      const mockActivities = getMockActivities();
      const totalMinutes = mockActivities.reduce((sum, a) => sum + a.duration, 0);
      ...
      parsed = {
        ...
        weeklyGoal: 150,
        monthlyGoal: 600,
        achievements: updateAchievements(getDefaultAchievements(), mockActivities),
        goals: getDefaultGoals(),
        currentStreak: 30,
        longestStreak: 30,
        totalSessions,
        averageSessionLength: Math.round(totalMinutes / totalSessions)
      };
    }
