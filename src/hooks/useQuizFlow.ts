...
import { performAyurvedicAnalysis, FullAnalysisResult } from "@/services/ayurvedicAnalysis";

// `performAyurvedicAnalysis` returns the wider `FullAnalysisResult`
// (where `vikriti` may be a string or a structured object). The narrower
// `AnalysisResult` interface used to live here was incompatible — alias
// to FullAnalysisResult so consumers see exactly what the analyzer returns.
type AnalysisResult = FullAnalysisResult;
...
export function useQuizFlow(questions: Question[]) {
  ...
  const next = useCallback((option: { value: string }) => {
    setState(prev => {
      const newAnswers = {
        ...prev.answers,
