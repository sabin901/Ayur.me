// Comprehensive Yoga Poses Data and Helper Functions
// Based on David Frawley's "Yoga for Your Type" and classical Ayurvedic texts
...
// Helper function to determine guna from pose characteristics
function getGunaFromPose(pose: any): 'Sattvic' | 'Rajasic' | 'Tamasic' {
  const description = pose.description.toLowerCase();
  const benefits = pose.benefits.join(' ').toLowerCase();
  const category = pose.category.toLowerCase();
  
  // Sattvic indicators
  if (description.includes('meditation') || description.includes('breathing') || 
      description.includes('calm') || description.includes('spiritual') ||
      benefits.includes('meditation') || benefits.includes('mindfulness')) {
    return 'Sattvic';
  }
  
  // Rajasic indicators
  if (description.includes('energetic') || description.includes('dynamic') ||
      description.includes('strength') || description.includes('power') ||
      benefits.includes('energizing') || benefits.includes('stimulating')) {
    return 'Rajasic';
  }
  
  // Tamasic indicators
  if (description.includes('restorative') || description.includes('relaxing') ||
      description.includes('gentle') || description.includes('therapeutic') ||
      benefits.includes('calming') || benefits.includes('grounding')) {
    return 'Tamasic';
  }
  
  // Default based on difficulty
  if (pose.difficulty === 'Beginner') return 'Tamasic';
  if (pose.difficulty === 'Advanced') return 'Rajasic';
  return 'Sattvic';
}

// Helper function to get dosha color
function getDoshaColor(dosha: string): string {
  const colors = {
    'Vata': 'bg-sage/20 text-sage-800 border-sage-300',
    'Pitta': 'bg-orange/20 text-orange-800 border-orange-300',
    'Kapha': 'bg-green/20 text-green-800 border-green-300',
    'All Doshas': 'bg-blue/20 text-blue-800 border-blue-300'
  };
  return colors[dosha as keyof typeof colors] || 'bg-gray/20 text-gray-800 border-gray-300';
}

// Helper function to get guna color
function getGunaColor(guna: string): string {
  const colors = {
    'Sattvic': 'from-sage/20 to-gold/20',
    'Rajasic': 'from-gold/20 to-sage/20',
    'Tamasic': 'from-sage/30 to-gold/30'
  };
  return colors[guna as keyof typeof colors] || 'from-sage/20 to-gold/20';
}

// Enhanced yoga poses with additional fields
export const comprehensiveYogaPoses: YogaPose[] = yogaPosesData.map((pose, index) => ({
  ...pose,
  id: `pose-${index}`,
  guna: getGunaFromPose(pose),
  gunaColor: getGunaColor(getGunaFromPose(pose)),
  doshaColor: getDoshaColor(pose.dosha),
  // Map instructions to stepByStep format
  stepByStep: pose.instructions.map((instruction, stepIndex) => ({
    step: stepIndex + 1,
    instruction: instruction,
    image: pose.image
  })),
  // Ensure all required fields are present
  therapeuticUses: pose.therapeuticUses || [],
  contraindications: pose.contraindications || [],
  sources: pose.sources || [],
  image: pose.image || "/yoga-poses/placeholder.svg"
}));
...
export const getPosesByDosha = (dosha: string): YogaPose[] => {
  return comprehensiveYogaPoses.filter(pose => pose.dosha === dosha || pose.dosha === 'All Doshas');
};

export const getPosesByCategory = (category: string): YogaPose[] => {
  return comprehensiveYogaPoses.filter(pose => pose.category === category);
};
...
export const getPosesByGuna = (guna: string): YogaPose[] => {
...
export const searchPoses = (query: string): YogaPose[] => {
  ...
  return comprehensiveYogaPoses.filter(pose =>
    ...
    pose.therapeuticUses.some(use => use.toLowerCase().includes(lowercaseQuery))
  );
};
