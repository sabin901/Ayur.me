...
export default function DiseaseDatabasePage() {
    ...
    fetch(`${API_BASE_URL}/diseases?limit=1000`, { signal: controller.signal })
      ...
      .then((data) => {
        ...
        const existingNames = new Set(
          backendDiseases.map((d) => d.name?.toLowerCase())
        );
      ...
      .catch((err) => {
        ...
        // No backend (e.g. Vercel-only deployment) — fall back to bundled
        // curated data so the page is still useful. We keep the user-facing
        // error null because the fallback is a successful experience.
  ...
  const clearFilters = () => {
    ...
    <div className="min-h-screen bg-gradient-to-r from-earth/10 to-sage/10">
      <h1 className="sr-only">Ayurvedic Disease Knowledge</h1>
          ...
          <div className="w-full">
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-orange-500 mb-2">Unable to load diseases</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredDiseases.map((disease, index) => (
                  <div
                    key={disease._id || disease.id || `${disease.name}-${index}`}
                  ...
                  >
                    ...
                    <CardHeader className="pb-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(disease.dosha || disease.primaryDosha || []).map((dosha, doshaIndex) => (
                          <Badge key={`${dosha}-${doshaIndex}`} variant="outline" className={`${doshaColors[dosha] || "bg-earth/20 text-foreground"} border-2 border-earth/30`}>
                            {dosha}
                          </Badge>
                        ))}
