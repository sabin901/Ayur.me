...
class DoshaAnalysisService {
  ...
  calculateDoshaScores(answers) {
    ...
    // Calculate percentages — guard against zero so we don't return NaN
    // values to the client when the assessment had no scoring options.
  ...
  async performComprehensiveAssessment(assessmentData) {
    ...
    // Persist the assessment only when we have a real authenticated
    // user — the schema requires a real ObjectId for `userId`, and the
    // public `/assess` endpoint can be called anonymously.
    ...
    if (userId && userId !== 'anonymous' && mongoose.Types.ObjectId.isValid(userId)) {
      try {
        ...
        // Don't fail the whole assessment if persistence fails — the
        // analysis is still useful to the caller. The error is logged
        // for ops but not surfaced to the user.
        // eslint-disable-next-line no-console
        console.error('PrakritiAssessment.save failed:', err.message);
      }
    }
