...
const quickAssessSchema = Joi.object({
 ...
 * @access Public
 */
router.get('/assessment/:assessmentId', optionalAuth, async (req, res, next) => {
  try {
    ...
    // If the assessment is owned by a user, only that user can view it.
    if (assessment.userId && (!req.user || String(req.user.id) !== String(assessment.userId))) {
 ...
 * @access Public
 */
router.post('/quick-assess', async (req, res, next) => {
  try {
    const { error, value } = quickAssessSchema.validate(req.body);
    if (error) {
      return res.status(400).json({
        error: 'Validation error',
        details: error.details.map(d => d.message)
    ...
    const result = doshaAnalysisService.calculateDoshaScores(value.answers);
